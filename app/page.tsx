...
function AccountSetup({ onComplete }: { onComplete: (state: FamilyState) => void }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (age >= 18) {
      ...
      onComplete({ accounts: [account], families: [{ id: familyId, name: familyName.trim(), memberIds: [accountId], guardianIds: [accountId] }], currentAccountId: accountId, activeFamilyId: familyId });
      return;
    }
...
function AccountLogin({ state, onSuccess, onlineUser }: { state: FamilyState; onSuccess: (state: FamilyState) => void; onlineUser: OnlineUser | null }) {
  const [familyId, setFamilyId] = useState(""); const [familyName, setFamilyName] = useState(""); const [selected, setSelected] = useState(""); const [pin, setPin] = useState(""); const [error, setError] = useState("");
